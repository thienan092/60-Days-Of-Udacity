...
def noisy_max(teacher_preds, privacy_loss_lv, n_labels): 
    ...
    for i in range(teacher_preds.shape[1]):
        ...
        noisy_count = label_counts + np.random.laplace(np.zeros(len(label_counts)), np.ones(len(label_counts))/privacy_loss_lv, len(label_counts))
        indices.append(np.argmax(label_counts))
    return np.array(indices)